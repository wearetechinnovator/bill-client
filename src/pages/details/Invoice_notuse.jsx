...
const Invoice = () => {
            ...
            try {
                ...
                if (urlRoute) {
                    ...
                    if (req.status === 200) {
                        ...
                        setBillNumber(
                            res.data?.quotationNumber || res.data?.proformaNumber || res.data?.poNumber ||
                        ...
                        setBillDate(
                            ...
                            res.data?.creditNoteDate || res.data?.purchaseReturnDate || res.data?.chalanDate
                        );
    ...
    useEffect(() => {
        ...
        billData.items.forEach((b) => {
            ...
            if (hsnMap[hsn]) {
                ...
                hsnMap[hsn] = { hsn, rate: taxRate, taxableValue, taxAmount };
            }
        });
    ...
    // ─── PRINT-JS: Download Bill (exact HTML render) ──────────────────────────
    const downloadBill = (filename = "invoice") => {
        try {
            ...
            printJS({
                printable: 'mainBill',
                type: 'html',
                targetStyles: ['*'],       // copy ALL styles from page
                scanStyles: true,
                maxWidth: 794,             // A4 at 96dpi
                documentTitle: filename,
                // Inject extra print CSS to keep it pixel-perfect
                style: `
                    #mainBill {
                        padding: 16px;
                        font-family: inherit;
                    ...
                    #mainBill .border   { border: 0.5px solid #94a3b8 !important; }
                    #mainBill .border-b { border-bottom: 0.5px solid #94a3b8 !important; }
                    #mainBill .border-r { border-right: 0.5px solid #94a3b8 !important; }
                    #mainBill .border-l { border-left: 0.5px solid #94a3b8 !important; }
                    #mainBill .border-t { border-top: 0.5px solid #94a3b8 !important; }
                    #mainBill .bg-gray-100 { background-color: #F3F4F6 !important; }
                    #mainBill .cancel__invoice {
                        position: absolute;
                        top: 50%;
                        left: 50%;
                ...
                `,
                onLoadingEnd: () => setDownloadLoading(false),
                onError: (err) => {
                    console.error(err);
                    toast("Invoice could not be downloaded", 'error');
    ...
    // ─── Send bill via Mail (html2canvas fallback for base64) ─────────────────
    // Mail ke liye base64 chahiye — isliye html2canvas use karna padega
    // Ya aap backend Puppeteer use kar sakte ho better quality ke liye
    const sendViaMail = async () => {
        try {
            // Dynamically import html2canvas only when needed (mail use case)
            const html2canvas = (await import('html2canvas')).default;
            const { default: jsPDF } = await import('jspdf');

            const element = document.getElementById('mainBill');
    ...
    const printBill = () => {
        ...
        style.innerHTML = `
            @media print {
                ...
                #mainBill, #mainBill * { visibility: visible; }
                #mainBill {
                    position: absolute;
                    left: 0; top: 0;
                    border: none;
            ...
            <main id='main'>
                ...
                {bill === "salesinvoice" && (
                    <PaymentInModal
                        ...
                        openStatus={() => setPaymentModal(false)}
                    />
                )}
                                    ...
                                    <div id='invoiceBtn' className='flex gap-2 items-center'>
                                        ...
                                        <Whisper
                                            ...
                                            speaker={
                                                <Popover>
                                                    <div
                                                        onClick={() => { sendViaMail(); setShareDrpdwn(false); }}
                                        ...
                                        >
                                            <div className='flex items-center gap-3 bg-[#003E32] text-white rounded-[5px] px-2 py-[5px] cursor-pointer'>
                                    ...
                                    <div className='flex items-center gap-4'>
                                        ...
                                        {paymentButtonShow && (bill === "salesinvoice" || bill === "purchaseinvoice") && (
                                            <button className='payment__button' onClick={() => setPaymentModal(true)}>
                                                ...
                                                {bill === "purchaseinvoice" ? "Record Payment Out" : "Record Payment In"}
                                            </button>
                                        )}
                                                ...
                                                speaker={
                                                    <Popover full>
                                                        ...
                                                        <div
                                                            ...
                                                            onClick={() => {
                                                                swal({ title: "Are you sure?", icon: "warning", buttons: true })
                                                                    .then((cnv) => {
                                                                        if (cnv) {
                                                                            swal("Quotation successfully duplicate", { icon: "success" });
                                                        ...
                                                        <div className='download__menu' onClick={printBill}>
                                                            <Icons.PRINTER className="text-[15px]" />
                                                            Print
                                                        </div>
                                                        {Number(billData?.paymentAmount || 0) <= 0 && billData?.isCancel === false && (
                                                            <div className='download__menu' onClick={() => setOpenConfirm(true)}>
                            ...
                            {!loading ? (
                                <div id='mainBill' className='border border-slate-600 rounded p-4'>
                                    <div ref={downloadRef} id='invoice'>
                                        ...
                                        <div className='border border-b-0 w-full mt-3 relative'>
                                            ...
                                            <div className='flex w-full border-b'>
                                                ...
                                                <div className='flex flex-col justify-center px-3' style={{ fontSize: '12px', width: '40%' }}>
                                                    ...
                                                    <p><span className='font-semibold'>{billName} Date: </span>{new Date(billDate).toLocaleDateString()}</p>
                                                </div>
                                            </div>
                                        ...
                                        <div className='table__wrapper items-page'>
                                            <table className='w-full border item__table' style={{ fontSize: '12px' }}>
                                                <thead className='bg-gray-100'>
                                                    <tr>
                                                        <td align='center' valign='middle' className='p-2' width={"4%"}>SL.NO</td>
                                                        <td align='center' width={"38%"}>ITEM</td>
                                                        <td align='center' width={"8%"}>HSN/SAC</td>
                                                ...
                                                <tbody>
                                                    {billData && billData.items.map((data, index) => (
                                                        <tr key={data._id}>
                                                            <td valign='middle' align='center' className='p-2 border'>{index + 1}</td>
                                                            <td valign='middle' align='left'>
                                                                {data.itemName}
                                                                {data.description && (
                                                                    <p className='text-gray-500 text-[10px] mt-1'>{data.description}</p>
                                                                )}
                                                            </td>
                                                            <td valign='middle' align='right'>{data.hsn}</td>
                                                            <td valign='middle' align='right'>{data.qun} <sub>{data.selectedUnit}</sub></td>
                                                            <td valign='middle' align='right'>{data.price}</td>
                                                            ...
                                                            <td valign='middle' align='right' style={{ whiteSpace: 'nowrap' }}>
                                                ...
                                                <tfoot className='w-full'>
                                                    <tr className='font-bold' style={{ background: "#F3F4F6" }}>
                                                        ...
                                                        <td></td>
                                                        <td style={{ whiteSpace: 'nowrap' }}><Icons.RUPES className='inline' />{billDetails.discount}</td>
                                                        <td style={{ whiteSpace: 'nowrap' }}><Icons.RUPES className='inline' />{billDetails.taxAmount}</td>
                                                        <td style={{ whiteSpace: 'nowrap' }}><Icons.RUPES className='inline' />{billDetails.amount}</td>
                                                    </tr>
                                                    {billData?.roundOffAmount && (
                                                        <tr className='font-semibold' style={{ background: "#F3F4F6" }}>
                                                            ...
                                                            <td style={{ whiteSpace: 'nowrap' }}>
                                                                <Icons.RUPES className='inline' />
                                                                {billData.roundOffType === "0" ? "-" + billData?.roundOffAmount : billData?.roundOffAmount}
                                                            </td>
                                                        </tr>
                                                    )}
                                                    {billData?.roundOffAmount && (
                                                    ...
                                                    <tr className='font-semibold' style={{ background: "#F3F4F6" }}>
                                                        ...
                                                        <td style={{ whiteSpace: 'nowrap' }}>
                                                            <Icons.RUPES className='inline' />{billData?.paymentAmount || "0.00"}
                                                        </td>
                                                    </tr>
                                                    <tr className='font-semibold' style={{ background: "#F3F4F6" }}>
                                            ...
                                            <table className='w-full' style={{ fontSize: '12px' }}>
                                                <thead className='bg-gray-100'>
                                                    <tr>
                                                        ...
                                                        <td>Tax Type</td>
                                                        <td>Rate</td>
                                                        {companyDetails?.state === billData?.party.state && <td>Amount</td>}
                                                ...
                                                <tbody>
                                                    ...
                                                    {hsnData && companyDetails?.state !== billData?.party.state && (() => {
                                                        ...
                                                        for (let i = 0; i < hsnData.length; i++) {
                                                            ...
                                                            seen[data.hsn] = true;
                                                            const igstAmount = (data.taxableValue * data.rate / 100).toFixed(2);
                                                            rows.push(
                                                                <tr key={`${i}-igst`}>
                                                                    <td>{data.hsn}</td>
                                                    ...
                                                    {hsnData && companyDetails?.state === billData?.party.state && (
                                                        [...new Map(hsnData.map(item => [item.hsn, item])).values()].map((data, i) => {
                                                            ...
                                                            const sgstAmount = (data.taxableValue * halfRate / 100).toFixed(2);
                                                            const cgstAmount = (data.taxableValue * halfRate / 100).toFixed(2);
                                                            const totalTax = (Number(sgstAmount) + Number(cgstAmount)).toFixed(2);
                                                            return (
                                                                <React.Fragment key={`${i}-cgst`}>
                                                                    <tr>
                                                                        <td rowSpan={2}>{data.hsn}</td>
                                                                        <td rowSpan={2}>{data.taxableValue.toFixed(2)}</td>
                                                                        <td>SGST</td>
                                                                        <td>{halfRate}%</td>
                                                                        <td>{sgstAmount}</td>
                                        ...
                                        {/* Footer: Amount in Words + Bank + Notes + Signature */}
